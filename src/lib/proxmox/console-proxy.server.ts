...
function forward(socket: WebSocket | undefined, data: MessageEvent["data"]) {
  if (socket?.readyState === WebSocket.OPEN) socket.send(data as WebSocketSendData);
}
...
export async function handleProxmoxConsoleRequest(request: Request): Promise<Response> {
  ...
  server.addEventListener("message", async (event) => {
    ...
    try {
      ...
      if (!termRes.ok) throw new Error(`Proxmox API termproxy → ${termRes.status}`);
      ...
      sendTerminalError(server, error instanceof Error ? error.message : "Erreur de connexion WebSocket.");
      closeSocket(server);
    }
  });
